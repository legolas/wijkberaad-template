...
<?php
	...
	<div class="acymailing_fulldiv" id="acymailing_fulldiv_<?php echo $formName; ?>" <?php echo $styleString; ?> >
		...
		<div class="acymailing_module_form" >
			...
			<div class="acymailing_form">
				...
				if(empty($identifiedUser->userid) AND $config->get('captcha_enabled') AND acymailing_level(1)){ ?>
					...
					if(version_compare(JVERSION,'1.6.0','>=')){
						echo '<img id="captcha_picture_'.$formName.'" width="'.$config->get('captcha_width_module').'" height="'.$config->get('captcha_height_module').'" class="captchaimagemodule" src="'.JRoute::_('index.php?option=com_acymailing&ctrl=captcha&acyformname='.$formName.'&val='.rand(0,10000).'&no_html=1').'" alt="captcha" />';
					}else{
						echo '<img id="captcha_picture_'.$formName.'" width="'.$config->get('captcha_width_module').'" height="'.$config->get('captcha_height_module').'" class="captchaimagemodule" src="'.rtrim(JURI::root(),'/').'/index.php?option=com_acymailing&amp;ctrl=captcha&amp;acyformname='.$formName.'&amp;val='.rand(0,10000).'&amp;no_html=1" alt="captcha" />';
					}?>
					<span><input id="user_captcha_<?php echo $formName; ?>" title="<?php echo JText::_('ERROR_CAPTCHA'); ?>" class="inputbox captchafield" type="text" name="acycaptcha" size="5" /></span>
					</p>
				<?php }
			...
			<?php if(!empty($postText)) echo '<span class="acymailing_finaltext">'.$postText.'</span>';
